...
export default function ClassesPage({ type }: { type?: "group" | "one-to-one" }) {
  ...
  const startInstantOneToOne = trpc.class.startInstantOneToOne.useMutation({
    onSuccess: (data) => {
      toast.success("1-to-1 Session started instantly");
      oneToOneQuery.refetch();
      allocationsQuery.refetch();
      if (data) {
            ...
            <>
              ...
              <div className="block md:hidden space-y-4 p-4">
                {upcoming.map((cls) => {
                  ...
                  return (
                    <Card key={cls.id} className="border border-slate-100 shadow-sm rounded-xl overflow-hidden p-4 space-y-2 bg-white">
                      <div className="flex items-center justify-between">
                        <span className="font-semibold text-emerald-800 text-xs uppercase">{cls.classType === "one_to_one" ? "One-on-One" : "Group"}</span>
                        <Badge className="bg-emerald-50 text-emerald-700 border border-emerald-200 hover:bg-emerald-50 text-[9px] font-bold uppercase">
                          {cls.status === "reschedule_request_pending" ? "Reschedule Pending" : cls.status === "rescheduled" ? "Rescheduled" : "Scheduled"}
                        </Badge>
                      </div>
                      <div className="text-sm font-semibold text-slate-800">{dateStr}</div>
                      <div className="text-xs text-slate-500 font-mono">{startTimeStr} - {endTimeStr}</div>
                      <div className="space-y-1.5 pt-2 border-t border-slate-100 text-xs">
                        <div className="flex justify-between"><span className="text-gray-500">Course/Module:</span> <span className="font-medium text-gray-800">{courseName}</span></div>
                        {sessionTypeFilter === "group" && (
                          <div className="flex justify-between"><span className="text-gray-500">Batch:</span> <span className="font-medium text-gray-800">{batchName}</span></div>
                        )}
                        <div className="flex justify-between"><span className="text-gray-500">Teacher:</span> <span className="font-medium text-gray-800">{cls.teacher?.name || "Unassigned"}</span></div>
                        {sessionTypeFilter === "one_to_one" && (
                          <div className="flex justify-between"><span className="text-gray-500">Student:</span> <span className="font-semibold text-slate-800">{cls.student?.name || "-"}</span></div>
                        )}
                      </div>
                    </Card>
                  );
                    ...
                    {hasMeeting ? (
                      ...
                      <div className="flex flex-wrap gap-2">
                        {card.sessionType === "one_to_one" && (
                          <Button
                            size="sm"
                            className="bg-[#C8102E] hover:bg-[#A50C22] text-white text-xs rounded-xl font-medium"
                            onClick={() => {
                              startInstantOneToOne.mutate({
                                studentId: card.enrollmentRecord.studentId,
                                sessionLength: card.duration,
                              });
                            }}
                            disabled={card.remaining <= 0 || startInstantOneToOne.isPending}
                          >
                            <Play className="w-3.5 h-3.5 mr-1" /> Start Class Now
                          </Button>
                        )}
                        <Button
                          size="sm"
                  ...
                  <>
                    {hasMeeting ? (
                      ...
                      <div className="flex flex-wrap gap-2">
                        ...
                        <Button
                          size="sm"
                          className="bg-[#1D2A44] hover:bg-[#152035] text-white text-xs rounded-xl font-semibold"
                          onClick={() => {
                            setSelectedEnrollmentForSchedule(card.enrollmentRecord);
                            setSelectedScheduleDuration(card.duration);
                            setScheduleDate(new Date(Date.now() + 5.5 * 60 * 60 * 1000).toISOString().slice(0, 16));
