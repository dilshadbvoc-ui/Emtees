      ...
      <Card className="border border-slate-100 shadow-sm rounded-xl overflow-hidden">
        <CardContent className="p-0 overflow-x-auto">
          ...
          <div className="block md:hidden space-y-4 p-4">
            {usersQuery.data?.map((u) => (
              <Card key={u.id} className="border border-slate-100 shadow-sm rounded-xl overflow-hidden p-4 space-y-3 bg-white">
                <div className="flex items-center justify-between">
                  <span className="font-mono text-xs font-semibold text-gray-600">{u.unionId}</span>
                  <Badge variant={u.status === "active" ? "default" : "secondary"}>{u.status}</Badge>
                </div>
                <div>
                ...
                <div className="flex items-center justify-between pt-2 border-t border-slate-100">
                  ...
                  <div className="flex gap-1">
                    {u.role === "teacher" && (
                      <Button size="sm" variant="ghost" className="text-emerald-600 hover:text-emerald-700 h-8 w-8 p-0" onClick={() => setDetailsTeacherId(u.id)}>
