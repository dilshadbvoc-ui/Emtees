...
export default function ClosuresList() {
  ...
  return (
    <Card className="bg-slate-900 border-slate-800">
      <CardHeader>
        <CardTitle className="text-slate-100">Recent Closures</CardTitle>
      </CardHeader>
      <CardContent>
        <Table>
          <TableHeader className="bg-slate-800">
            <TableRow>
              ...
              <TableHead className="text-slate-300 text-right">Total Fee</TableHead>
          ...
          <TableBody>
            ...
            {closures?.map((closure) => (
              <TableRow key={closure.id} className="border-slate-800">
                <TableCell className="text-slate-300">{format(new Date(closure.closingDate), "MMM dd, yyyy")}</TableCell>
                <TableCell className="text-slate-200 font-medium">
                  {closure.studentName || "N/A"}
                  <div className="text-xs text-slate-500">{closure.admNo}</div>
                </TableCell>
                <TableCell className="text-slate-300">{closure.courseName}</TableCell>
                <TableCell>
                  <Badge variant="outline" className="bg-slate-800 text-indigo-400 border-indigo-900">
                    {closure.type}
                  </Badge>
                </TableCell>
                <TableCell className="text-right text-slate-200">₹{closure.totalFee}</TableCell>
