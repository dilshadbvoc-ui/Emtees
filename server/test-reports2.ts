...
import { fetchFullStudentReportData, fetchFullTeacherReportData } from "./src/routers/admin.js";

async function run() {
  const db = getDb();
  console.log("Testing student report...");
  try {
    const s = await fetchFullStudentReportData(db, 1); // might not be student
  ...
  try {
    const t = await fetchFullTeacherReportData(db, 2); // might not be teacher
