      ...
      try {
        const result = await db.transaction(async (tx) => {
          return await StudentAdmissionService.admitStudent(tx, {
            ...
            sessionType: input.oneOnOneEnabled && input.groupSessionEnabled ? "both" : input.oneOnOneEnabled ? "one_on_one" : "group",
            feesTotal: totalCourseFee,
            paymentType: input.paymentOption === "installment" ? "INSTALLMENT" : "FULL_PAYMENT",
        ...
        uniqueStudentId = result.unionId;
      } catch (err: any) {
        throw new TRPCError({ code: "BAD_REQUEST", message: err.message || "Failed to register student" });
      }
