...
export const userRouter = createRouter({
  ...
  getTeacherStats: teacherQuery.query(async ({ ctx }) => {
    ...
    const todayClasses = await db
      ...
      .where(
        and(
          eq(classes.teacherId, ctx.user.id),
          gte(classes.scheduledAt, startOfToday),
    ...
    const todayAttendanceList = await db
      ...
      .where(
        and(
          eq(attendance.teacherId, ctx.user.id),
    ...
    const todayAbsentCount = todayAttendanceList.filter(
      (a) => a.status === "absent"
    ).length;
  ...
  importStudents: adminQuery
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      for (const s of input) {
        ...
        // Find module by name (s.course is module name)
        let moduleId = 0;
        if (s.course) {
          const mod = await db.query.modules.findFirst({
            where: and(eq(modules.name, s.course), eq(modules.status, "active")),
          });
          if (mod) {
            moduleId = mod.id;
          }
        ...
        // Find batch by name (s.batch is batch name)
        let batchId: number | null = null;
        if (s.batch && moduleId) {
          const bat = await db.query.batches.findFirst({
            where: and(eq(batches.name, s.batch), eq(batches.moduleId, moduleId), eq(batches.status, "active")),
          });
          if (bat) {
            batchId = bat.id;
          }
        ...
        const tempPassword = parsed.phoneNumber.slice(-6);
        ...
        try {
          const result = await db.transaction(async (tx) => {
            return await StudentAdmissionService.admitStudent(tx, {
              ...
              phoneNumber: parsed.phoneNumber,
              email: s.email || null,
              username: parsed.phoneNumber, // Default username to phone number as before
              password: tempPassword,
              enrollmentId: s.enrollmentId || s.userId || null,
              courseId: moduleId,
              batchId: batchId,
              feesTotal: s.feesTotal || null,
          ...
          throw new TRPCError({
            code: "BAD_REQUEST",
            message: `Failed to import student ${s.name}: ${err.message || String(err)}`,
          });
        }
